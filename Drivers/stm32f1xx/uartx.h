...
void UARTX_IRQHandler(void)
{
  ...
  {                                     /* RX */
    udr = UARTX->DR;
    UARTX->SR &= ~USART_SR_RXNE;        /* clear interrupt */
  ...
  if (usr & USART_SR_TXE)
  {                                     /* TX */
    UARTX->SR &= ~USART_SR_TXE;         /* clear interrupt */
...
void uartx_init(void)
{
  ...
  RCC->APB2ENR |= GPIOX_CLOCK(UARTX_RX) | GPIOX_CLOCK(UARTX_TX) | RCC_APB2ENR_AFIOEN;
  GPIOX_MODE(MODE_FF_DIGITAL_INPUT, UARTX_RX);
  GPIOX_MODE(MODE_PP_ALTER_50MHZ, UARTX_TX);
  #elif GPIOX_PORTNUM(UARTX_RX) >= GPIOX_PORTNUM_A
  #define UARTX_CR1_RXNEIE       USART_CR1_RXNEIE
  #define UARTX_CR1_RE           USART_CR1_RE
  #define UARTX_CR1_TE           0
  RCC->APB2ENR |= GPIOX_CLOCK(UARTX_RX) | RCC_APB2ENR_AFIOEN;
  GPIOX_MODE(MODE_FF_DIGITAL_INPUT, UARTX_RX);
  #else
  #define UARTX_CR1_RXNEIE       0
  #define UARTX_CR1_RE           0
  #define UARTX_CR1_TE           USART_CR1_TE
  RCC->APB2ENR |= GPIOX_CLOCK(UARTX_TX) | RCC_APB2ENR_AFIOEN;
  GPIOX_MODE(MODE_PP_ALTER_50MHZ, UARTX_TX);
  #endif

  #if UARTX_REMAP == 1
  AFIO->MAPR |= 1 < UARTX_MAPR;
  #elif UARTX_REMAP == 3
  AFIO->MAPR |= 3 < UARTX_MAPR;
  #endif
